...
const BlogSection = () => {
  ...
  useEffect(() => {
    contentApi
      .list<Post>('blog')
      .then((items) => items.length && setPosts(items))
      .catch(() => undefined);
  }, []);
              ...
              <CardContent className="p-6 space-y-3">
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <Icon name="Calendar" size={14} /> {p.date ?? fmtDate(p.created_at)}
                  </span>
                  <span className="flex items-center gap-1">
                    <Icon name="Clock" size={14} /> {p.read ?? p.read_time}
                  </span>
                </div>
